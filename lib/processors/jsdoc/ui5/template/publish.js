...
const ANONYMOUS_LONGNAME = doclet.ANONYMOUS_LONGNAME;
const A_SECURITY_TAGS = [
	{
		name: "SecSource",
	...
	},
	{
		name: "SecValidate",
		caption: "Validation",
		description: "APIs that (partially) cleanse tainted data so that it no longer poses a security risk " +
			"in the further data flow of an application",
		params: ["in", "out", "flags"]
	}
];

const	rSecurityTags = new RegExp(A_SECURITY_TAGS.map(function($) {
	return $.name.toLowerCase();
}).join("|"), "i");
// debug(A_SECURITY_TAGS.map(function($) {return $.name; }).join('|'));

const templateConf = (env.conf.templates || {})[MY_TEMPLATE_NAME] || {};


const pluginConf = templateConf;
 ...
 * This map is maintained to deal with names that only differ in case (e.g. the namespace sap.ui.model.type
 * and the class sap.ui.model.Type).
 */
...
function isaClass($) {
	return /^(namespace|interface|class|typedef)$/.test($.kind) || ($.kind === "member" && $.isEnum);
}
...
const Version = (function() {
	 ...
	 * <ul>
	 * <li>Version("1.2.3-SNAPSHOT") - as a dot-separated string. Any non-numerical char or a dot
	 * followed by a non-numerical char starts the suffix portion.
	 * Any missing major, minor or patch versions will be set to 0.</li>
	 * <li>Version(1,2,3,"-SNAPSHOT") - as individual parameters. Major, minor and patch must be integer numbers or
	 * empty, suffix must be a string not starting with digits.</li>
	 * <li>Version([1,2,3,"-SNAPSHOT"]) - as an array with the individual parts. The same type restrictions
	 * apply as before.</li>
	 * <li>Version(otherVersion) - as a Version instance (cast operation). Returns the given instance instead of
...
function formatText(text) {
	...
	text = String(text).replace(rFormatText,
		function(match, pre, endpre, flow, endflow, linkTarget, linkText, emptyline) {
			if ( pre ) {
				inpre = true;
				return pre.replace(/<pre>/gi, "<pre class=\"prettyprint\">").replace(/<pre\s+lang="([^"]+)"\s*>/gi,
...
//	summarize("This is a first\n\nparagraph with empty \n   \n   \nlines in it. This is the remainder.")));

function childrenOfKind(data, kind) {
	/* old version based on TaffyDB (slow)
	var oChildren = symbolSet({
		kind: kind,
		memberof: data.longname === GLOBAL_LONGNAME ? {isUndefined: true} : data.longname}).filter(function() {
			return conf.filter(this);
		});
	return {
 ...
 * to the set.
 *
 * @param {Any} symbol of which these are the members
...
function groupByContributors(symbol, aBorrowedMembers) {
	...
	(function handleAugments(oSymbol) {
		let i; let oTarget; let aParentsToVisit;
		if ( oSymbol.augments ) {
			aParentsToVisit = [];
			// first assign an order
			for (i = 0; i < oSymbol.augments.length; i++) {
				if ( mContributors[oSymbol.augments[i]] != null &&
	...
	for (i in mContributors) {
		if (mContributors.hasOwnProperty(i)) {
...
function TypeParser(defaultBuilder) {
	 ...
	 */
	const rLexer = /\s*(Array\.?<|Object\.?<|Set\.?<|Promise\.?<|function\(|\{|:|\(|\||\}|>|\)|,|\[\]|\*|\?|!|\.\.\.)|\s*(\w+(?:[.#~]\w+)*)|./g;
	...
	function next(expected) {
		if ( expected !== undefined && token !== expected ) {
			throw new SyntaxError("TypeParser: expected '" + expected + "', but found '" + tokenStr + "' (pos: " +
				rLexer.lastIndex + ", input='" + input + "')");
		}
		...
		if ( match ) {
			...
			if ( !token ) {
				throw new SyntaxError("TypeParser: unexpected '" + tokenStr + "' (pos: " + match.index + ", input='" +
					input + "')");
			}
		} else {
			tokenStr = token = null;
	...
	function parseType() {
		...
		if ( token === "Array.<" || token === "Array<" ) {
			...
			next();
			let thisType; let constructorType; const paramTypes = []; let returnType;
			if ( tokenStr === "this" ) {
				next();
				next(":");
				thisType = parseType();
				...
				if ( token === "," ) {
					if ( repeatable ) {
						throw new SyntaxError(
							"TypeParser: only the last parameter of a function can be repeatable (pos: " +
							rLexer.lastIndex + ", input='" + input + "')");
					}
					next();
				}
			...
			let propName; let propType;
			next();
			do {
				propName = tokenStr;
				if ( !/^\w+$/.test(propName) ) {
					throw new SyntaxError(
						"TypeParser: structure field must have a simple name (pos: " + rLexer.lastIndex +
						", input='" + input + "', field:'" + propName + "')");
				}
				next("symbol");
				if ( token === ":" ) {
					next();
...
TypeParser.LinkBuilder.prototype = {
	...
	function: function(paramTypes, returnType) {
		return {
			str: "function(" + paramTypes.map(function(type) {
				return type.str;
			}).join(",") + ")" + ( returnType ? " : " + this.safe(returnType) : "")
		};
	},
	structure: function(structure) {
		const r = [];
		for ( const fieldName in structure ) {
...
const _SHORT_BUILDER = new TypeParser.LinkBuilder("short", true);
const _LONG_BUILDER = new TypeParser.LinkBuilder("long", true);
const _TEXT_BUILDER = new TypeParser.LinkBuilder("text", false);
const _TEXT_BUILDER_ENCODED = new TypeParser.LinkBuilder("text", true);
...
function asPlainSummary(sText) {
	return sText ? summarize(sText).replace(/<.*?>/g, "").replace(/\{\@link\s*(.*?)\}/g, "$1")
		.replace(/"/g, "&quot;") : "";
}
